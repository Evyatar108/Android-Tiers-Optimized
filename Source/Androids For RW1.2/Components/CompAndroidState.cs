            ...
            if (GT % 300 == 0)
            {
                ...
                if (csm != null && csm.Infected == -1)
                {
                    ...
                    if (Utils.androidIsValidPodForCharging(cp) && !isOrganic)
                    {
                        ...
                        cp.needs.food.CurLevelPercentage += Settings.percentageOfBatteryChargedEach12Sec;
        ...
        public void checkTXWithSkinFacialTextureUpdate()
        {
            try
            {
                ...
                if (isAndroidWithSkin)
                {
                    ...
                    {
                        Traverse.Create(cp.story).Field("headGraphicPath").SetValue(Utils.lastResolveAllGraphicsHeadGraphicPath);
                        Utils.lastResolveAllGraphicsHeadGraphicPath = null;
                        /*cp.Drawer.renderer.graphics.ResolveAllGraphics();
                        PortraitsCache.SetDirty(cp);*/
                    }
                ...
                if (Utils.POWERPP_LOADED && useBattery)
                {
                    ...
                    yield return new Command_Action
                    {
                        ...
                        action = delegate ()
                        {
                            ...
                            for (int i = 0; i < parent.Map.listerBuildings.allBuildingsColonist.Count; i++)
                            {
                                Building build = parent.Map.listerBuildings.allBuildingsColonist[i];
                        ...
                        action = delegate ()
                        {
                            ...
                            Utils.ShowFloatMenuAndroidCandidate((Pawn)parent, delegate (Pawn target)
                            {
                                Find.WindowStack.Add(
                                    new Dialog_Msg("ATPP_UploadConsciousnessConfirm".Translate(parent.LabelShortCap, target.LabelShortCap), $"{"ATPP_UploadConsciousnessConfirmDesc".Translate(parent.LabelShortCap, target.LabelShortCap)}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                                    {
