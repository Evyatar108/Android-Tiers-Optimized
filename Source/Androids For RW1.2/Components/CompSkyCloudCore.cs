    ...
    public class CompSkyCloudCore : ThingComp
    {
        public override void PostExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                foreach (var storedMind in storedMinds)
                    if (storedMind == null)
                        storedMinds.Remove(storedMind);
            }
        }
        ...
        public override void CompTick()
        {
            ...
            if (CGT % 120 == 0)
            {
                ...
                if (migrationsDone != null)
                {
                    for (int i = 0; i < migrationsDone.Count; i++)
                    {
                        CompSurrogateOwner md = migrationsDone[i];
                        md.OnMigrated();
                    }
                ...
                if (replicationDone != null)
                {
                    for (int i = 0; i < replicationDone.Count; i++)
                    {
                        CompSurrogateOwner md = replicationDone[i];
                        md.OnReplicate();
                    }
        ...
        public HashSet<Pawn> storedMinds = new HashSet<Pawn>();
        public Dictionary<Pawn, Building> controlledTurrets = new Dictionary<Pawn, Building>();
        public HashSet<Pawn> assistingMinds = new HashSet<Pawn>();
        public HashSet<Pawn> replicatingMinds = new HashSet<Pawn>();
