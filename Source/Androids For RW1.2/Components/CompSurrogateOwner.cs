    ...
    public class CompSurrogateOwner : ThingComp
    {
        public override void PostExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                availableSX.RemoveWhere(item => item == null);
            }
        }
            ...
            if (GT % 120 == 0 && (permuteEndingGT != -1 || duplicateEndingGT != -1 || uploadToSkyCloudEndingGT != -1 || migrationEndingGT != -1 || mindAbsorptionEndingGT != -1 || downloadFromSkyCloudEndingGT != -1 || (controlMode && SX != null)))
            {
                ...
                if (uploadToSkyCloudEndingGT != -1 && uploadToSkyCloudEndingGT < GT)
                {
                    ...
                    if (Settings.skyCloudUploadModeForSourceMind == 1)
                    {
                        ...
                        var hedifsToRemove = new List<Hediff>();
                        for (int i = 0; i < corpse.health.hediffSet.hediffs.Count; i++)
                        {
                            Hediff he = corpse.health.hediffSet.hediffs[i];
                            if (he.def == Utils.hediffHaveVX2Chip || he.def == Utils.hediffHaveVX3Chip)
                                hedifsToRemove.Add(he);
                        }

                        for (int i1 = 0; i1 < hedifsToRemove.Count; i1++)
                        {
                            Hediff hedifToRemove = hedifsToRemove[i1];
                            corpse.health.RemoveHediff(hedifToRemove);
                        }

                        BodyPartRecord bpr = corpse.health.hediffSet.GetBrain();
                ...
                if (mindAbsorptionEndingGT != -1 && mindAbsorptionEndingGT < GT)
                {
                    ...
                    int sum = 0;
                    for (int i = 0; i < cpawn.skills.skills.Count; i++)
                    {
                        SkillRecord sr = cpawn.skills.skills[i];
                        sum += sr.levelInt;
                    }
                    int average = (int)((float)sum / (float)cpawn.skills.skills.Count);
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
                    ...
                    yield return new Command_Action
                    {
                        ...
                        action = delegate ()
                        {
                            ...
                            Find.WindowStack.Add(
                                new Dialog_Msg("ATPP_MindAbsorptionConfirm".Translate(), $"{"ATPP_MindAbsorptionConfirmDesc".Translate(parent.LabelShortCap)}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                                {
            ...
            yield return new Command_Action
            {
                ...
                action = delegate ()
                {
                    ...
                    {
                        Find.WindowStack.Add(
                            new Dialog_Msg("ATPP_PermuteConsciousnessConfirm".Translate(parent.LabelShortCap, target.LabelShortCap), $"{"ATPP_PermuteConsciousnessConfirmDesc".Translate(parent.LabelShortCap, target.LabelShortCap)}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                            {
            ...
            yield return new Command_Action
            {
                ...
                action = delegate ()
                {
                    ...
                    {
                        Find.WindowStack.Add(
                            new Dialog_Msg("ATPP_DuplicateConsciousnessConfirm".Translate(parent.LabelShortCap, target.LabelShortCap), $"{"ATPP_DuplicateConsciousnessConfirmDesc".Translate(parent.LabelShortCap, target.LabelShortCap)}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                            {
            ...
            if (Utils.GCATPP.isThereSkyCloudCore())
            {
                ...
                yield return new Command_Action
                {
                    ...
                    action = delegate ()
                    {
                       ...
                       {
                           Find.WindowStack.Add(
                               new Dialog_Msg("ATPP_MoveToSkyCloud".Translate(), $"{"ATPP_MoveToSkyCloudDesc".Translate()}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                               {
                ...
                yield return new Command_Action
                {
                    ...
                    action = delegate ()
                    {
                        ...
                        Utils.ShowFloatMenuSkyCloudCores(delegate (Building target)
                        {
                            ...
                            {
                                Find.WindowStack.Add(
                                    new Dialog_Msg("ATPP_MoveFromSkyCloud".Translate(), $"{"ATPP_MoveFromSkyCloudDesc".Translate()}\n{"ATPP_WarningSkyMindDisconnectionRisk".Translate()}", delegate
                                {
            ...
            try
            {
                ...
                if (controlMode && availableSX.Count > 0)
                {
                    ...
                    foreach (var s in availableSX)
                    {
                        ...
                        if (SX == s)
                        {
                            lst.Append(Utils.getSavedSurrogateNameNick(SX));
                        }
                        else
                        {
                            lst.Append(Utils.getSavedSurrogateNameNick(s));
                        }
                    }

                    ret.AppendLine("ATPP_RemotelyControl".Translate(lst.ToString()));
                }
        ...
        public void setControlledSurrogate(Pawn controlled, bool external = false)
        {
            ...
            if (inMainSX)
            {
                ...
                {
                    savedSkillsBecauseM7Control = new List<string>(cp.skills.skills.Count);

                    for (int i = 0; i < cp.skills.skills.Count; i++)
                    {
                        SkillRecord s = cp.skills.skills[i];
                        string k = $"{s.def.defName}-{s.levelInt}-{s.xpSinceLastLevel}-{s.xpSinceMidnight}";
                        savedSkillsBecauseM7Control.Add(k);
                    }

                    if (cp.workSettings != null && cp.workSettings.EverWork)
                    {
                        savedWorkAffectationBecauseM7Control = new List<string>(DefDatabase<WorkTypeDef>.AllDefsListForReading.Count);
                        for (int i = 0; i < DefDatabase<WorkTypeDef>.AllDefsListForReading.Count; i++)
                        {
                            WorkTypeDef el = DefDatabase<WorkTypeDef>.AllDefsListForReading[i];
                            string k = $"{el.defName}-{cp.workSettings.GetPriority(el)}";
        ...
        public void stopControlledSurrogate(Pawn surrogate, bool externalController = false, bool preventNoHost = false, bool noPrisonedSurrogateConversion = false)
        {
            ...
            foreach (var csurrogate in availableSX)
            {
                ...
                try
                {
                    ...
                    {
                        for (int i = 0; i < savedSkillsBecauseM7Control.Count; i++)
                        {
                            string k = savedSkillsBecauseM7Control[i];
                            //Explode
                            string[] vals = k.Split('-');
                            if (vals.Length != 4)
                                continue;
                            ...
                            for (int i1 = 0; i1 < cp.skills.skills.Count; i1++)
                            {
                                SkillRecord s = cp.skills.skills[i1];
                                if (s.def == sd)
                                {
                                    s.levelInt = levelInt;
                        ...
                        {
                            for (int i = 0; i < savedWorkAffectationBecauseM7Control.Count; i++)
                            {
                                string k = savedWorkAffectationBecauseM7Control[i];
                                //Explode
                                string[] vals = k.Split('-');
                                if (vals.Length != 2)
                                    continue;
        ...
        public HashSet<Pawn> extraSX = new HashSet<Pawn>();
        ...
        public HashSet<Pawn> availableSX = new HashSet<Pawn>();
    }
}