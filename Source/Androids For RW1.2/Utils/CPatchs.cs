    ...
    public static class CPaths
    {
        ...
        public static void RimworldVanilla_PawnApparelGeneratorPossibleApparelSetPairOverlapsAnything(ThingStuffPair pair, ref bool __result, ThingDef ___raceDef, List<ThingStuffPair> ___aps)
        {
            try
            {
                ...
                {
                    for (int i = 0; i < ___aps.Count; i++)
                    {
                        ThingStuffPair el = ___aps[i];
        ...
        public static void PowerPP_CompLocalWirelessPowerEmitter_CompInspectStringExtra(ThingComp __instance, ref string __result)
        {
            ...
            int nbConn = 0;
            if (Utils.GCATPP.listerLWPNAndroid.TryGetValue(build, out var value))
                nbConn = value.Count;
        ...
        public static bool QEE_BuildingPawnVatGrower_TryMakeClonePrefix(Building __instance)
        {
            try
            {
                ...
                ThingOwner to = Traverse.Create(__instance).Field("innerContainer").GetValue<ThingOwner>();
        ...
        private static void QEE_CheckInitData(Building __instance)
        {
            ...
            if (!Utils.GCATPP.QEEAndroidHair.TryGetValue(BUID, out var value) || DefDatabase<HairDef>.GetNamed(value, false) == null)
            {
            ...
            try
            {
                ...
                b = new Command_Action()
                {
                    ...
                    action = delegate ()
                    {
                        ...
                        for (int i = 0; i < Utils.ExceptionHairColors.Length; i++)
                        {
                            string h = Utils.ExceptionHairColors[i];
                ...
                b = new Command_Action()
                {
                    ...
                    action = delegate ()
                    {
                        ...
                        for (int i = 0; i < Utils.ExceptionSkinColors.Length; i++)
                        {
                            string h = Utils.ExceptionSkinColors[i];
        ...
        public static void QEE_Building_GrowerBase_get_CraftingProgressPercentPostfix(Building __instance, ref float __result)
        {
            try
            {
                ...
                if (Utils.GCATPP.VatGrowerLastPawnIsTX.TryGetValue(BUID, out var value) && value && !Utils.GCATPP.VatGrowerLastPawnInProgress.ContainsKey(BUID))
                {
                    Utils.GCATPP.VatGrowerLastPawnInProgress[BUID] = (Pawn)Traverse.Create(__instance).Field("pawnBeingGrown").GetValue();
                }
                ...
                if (Utils.GCATPP.VatGrowerLastPawnInProgress.TryGetValue(BUID, out var pawn) && pawn != null)
                {
