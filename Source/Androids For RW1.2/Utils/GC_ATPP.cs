...
namespace MOARANDROIDS
{
    public class GC_ATPP : GameComponent
    {
        ...
        public GC_ATPP(Game game)
        {
            ...
            if (!Utils.init)
            {
                try
                {
                    ...
                    if (tcd != null && r != null)
                    {
                        for (int i = 0; i < tcd.childThingDefs.Count; i++)
                        {
                            ThingDef el = tcd.childThingDefs[i];
                            if (el == null)
                                continue;
                    ...
                    {
                        ThingDef td = DefDatabase<ThingDef>.AllDefsListForReading[i];
                        try
                        {
                            if (td != null && td.race != null)
                            {
                                ...
                                if (Utils.BIRDSANDBEES_LOADED && (Utils.ExceptionAndroidList.Contains(td.defName) || Utils.ExceptionAndroidAnimals.Contains(td.defName)))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (td.recipes != null)
                                        {
                                            List<RecipeDef> list = td.recipes.ToList();
                                            for (int i1 = 0; i1 < list.Count; i1++)
                                            {
                                                RecipeDef cr = list[i1];
        ...
        {
            HashSet<Hediff> toDel = new HashSet<Hediff>();
            ...
            foreach (var map in Find.Maps)
            {
                foreach (var p in map.mapPawns.AllPawns)
                {
                    if (p.IsAndroidTier() && p.health != null && p.health.hediffSet != null)
                    {
                        ...
                        if (toDel.Count > 0)
                        {
                            p.health.hediffSet.hediffs.RemoveAll(x => toDel.Contains(x));
                            toDel.Clear();
                        }
                    }
            ...
            foreach (var el in listerLWPNAndroid)
            {
                ...
                foreach (var android in el.Value.ToList())
                {
                    ...
                    else
                    {
                        ...
                        android.needs.food.CurLevelPercentage += Settings.percentageOfBatteryChargedEach12Sec;
        ...
        public void checkVirusedThings()
        {
            ...
            foreach (var t in listerVirusedThings)
            {
        ...
        public void pushSurrogateAndroid(Pawn sx)
        {
            ...
            if (!listerSurrogateAndroids.TryGetValue(MUID, out var value))
                listerSurrogateAndroids[MUID] = new HashSet<Pawn> { sx };
            else
                value.Add(sx);
        ...
        {
            if (!listerSkyMindServers.TryGetValue(build.Map, out var value))
            {
                value = new HashSet<Building>();
                listerSkyMindServers[build.Map] = value;
            }
        ...
        {
            if (listerSkyMindServers.TryGetValue(map, out var value))
            {
                if (value.Remove(build))
                    reProcessNbSlot();
            }
        ...
        {
            if (!listerSkyMindWANServers.TryGetValue(build.Map, out var value))
            {
                value = new HashSet<Building>();
                listerSkyMindWANServers[build.Map] = value;
            }

            if (!value.Contains(build) && build.TryGetComp<CompPowerTrader>().PowerOn)
            {
                value.Add(build);
                reProcessNbSlot();
            }
        ...
        {
            if (listerSkyMindWANServers.TryGetValue(map, out var value))
            {
                if (value.Remove(build))
                    reProcessNbSlot();
            }
        ...
        {
            if (!listerReloadStation.TryGetValue(build.Map, out var value))
            {
                value = new HashSet<Building>();
                listerReloadStation[build.Map] = value;
            }
        ...
        public Building getFreeReloadStation(Map map, Pawn android)
        {
            ...
            if (listerReloadStation.TryGetValue(map, out var value))
            {
                ...
                foreach (var el in value.OrderBy((Building b) => b.Position.DistanceToSquared(android.Position)))
                {
        ...
        public List<Thing> getHeatSensitiveDevicesByHotLevel(Map map, int hotLevel)
        {
            ...
            if (!listerHeatSensitiveDevices.TryGetValue(map, out var value))
                return null;
        ...
        void checkRemoveAndroidFactions()
        {
            ...
            if (androidFactionCoalition != null)
            {
                if (Settings.androidsAreRare)
                {
                    ...
                    {
                        foreach (var el in Find.WorldObjects.SettlementBases)
                        {
        ...
        {
            if (!listerHeatSensitiveDevices.TryGetValue(build.Map, out var value))
            {
                value = new HashSet<Building>();
                listerHeatSensitiveDevices[build.Map] = value;
            }
        ...
        {
            if (listerSecurityServers.Add(build))
        ...
        {
            if (listerSecurityServers.Remove(build))
                reProcessNbSecuritySlot();
        }

        public void pushSkillServer(Building build)
        {
            if (listerSkillServers.Add(build))
        ...
        {
            if (!listerSkillServers.Remove(build))
            {
                return;
            }
        ...
        {
            if (listerHackingServers.Add(build))
        ...
        {
            if (!listerHackingServers.Remove(build))
                return;
        ...
        {
            if (!listerSkyMindRelays.TryGetValue(build.Map, out var value))
            {
                value = new HashSet<Building>();
                listerSkyMindRelays[build.Map] = value;
            }
        ...
        {
            if (!listerSkyMindRelays.TryGetValue(map, out var value))
                return;
        ...
        {
            if (!listerLWPNAndroid.TryGetValue(LWPN, out var value))
            {
                value = new HashSet<Pawn>();
                listerLWPNAndroid[LWPN] = value;
            }
            ...
            if (LWPN != null
                && !LWPN.Destroyed
                && (LWPN.def.defName == "ARKPPP_LocalWirelessPowerEmitter" || (LWPN.def.defName == "ARKPPP_LocalWirelessPortablePowerEmitter" && value.Count < Settings.maxAndroidByPortableLWPN))
        ...
        private void initNull()
        {
            ...
            if (listerHeatSensitiveDevices == null)
                listerHeatSensitiveDevices = new Dictionary<Map, HashSet<Building>>();
            ...
            if (listerLWPNAndroid == null)
                listerLWPNAndroid = new Dictionary<Building, HashSet<Pawn>>();
        ...
        public Dictionary<Map, HashSet<Building>> listerHeatSensitiveDevices;
        ...
        public Dictionary<Building, HashSet<Pawn>> listerLWPNAndroid = new Dictionary<Building, HashSet<Pawn>>();
