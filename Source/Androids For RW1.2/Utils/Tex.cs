...
namespace MOARANDROIDS
{
    ...
    static class Tex
    {
        ...
        {
            Pair<string, Color> key = new Pair<string, Color>(string.Concat(type.ToString(), gender, front.ToString()), color);
            StringBuilder path = new StringBuilder();

            if (!eyeGlowEffectCache.TryGetValue(key, out Graphic res))
            {
                if (front == 1)
                {
                    if (gender == "M")
                        path.Append("Things/Misc/Androids/Effects/Front");
                    else
                        path.Append("Things/Misc/Androids/Effects/FFront");

                    eyeGlowEffectCache[key] = GraphicDatabase.Get<Graphic_Single>(path.Append(type).ToString(), ShaderDatabase.MoteGlow, Vector2.one, color);
                }
                else
                {
                    if (gender == "M")
                        path.Append("Things/Misc/Androids/Effects/Side");
                    else
                        path.Append("Things/Misc/Androids/Effects/FSide");
