...
namespace MOARANDROIDS
{
    public static class Utils
    {
        ...
        public static string[] BlacklistedHediffsForAndroids = new string[] { "Anxiety" };
        ...
        public static void removeAllSlowNetworkHediff(bool onlyInCaravan = false)
        {
            ...
            if (!onlyInCaravan)
            {
                ...
                {
                    Map m = Find.Maps[i];
                    for (int i1 = 0; i1 < m.mapPawns.FreeColonistsAndPrisoners.Count; i1++)
                    {
                        Pawn p = m.mapPawns.FreeColonistsAndPrisoners[i1];
            ...
            {
                RimWorld.Planet.Caravan e = Find.WorldObjects.Caravans[i];
                IList<Thing> list = e.pawns;
        ...
        public static Lord LordOnMapWhereFactionIsInvolved(Map map, Faction faction)
        {
            ...
            {
                Lord l = map.lordManager.lords[i];
        ...
        public static bool antennaSelected()
        {
            ...
            {
                object el = Find.Selector.SelectedObjects[i];
                if (el is Building)
                {
        ...
        public static void ShowFloatMenuAndroidCandidate(Pawn emitter, Action<Pawn> onClick)
        {
            ...
            for (int i = 0; i < emitter.Map.mapPawns.FreeColonistsAndPrisoners.Count; i++)
            {
                Pawn colon = emitter.Map.mapPawns.FreeColonistsAndPrisoners[i];
        ...
        public static void Duplicate(Pawn source, Pawn dest, bool overwriteAsDeath = true)
        {
            try
            {
                ...
                for (int i = 0; i < source.story.traits.allTraits.Count; i++)
                {
                    Trait trait = source.story.traits.allTraits[i];
                ...
                for (int i = 0; i < source.needs.mood.thoughts.memories.Memories.Count; i++)
                {
                    Thought_Memory x = source.needs.mood.thoughts.memories.Memories[i];
        ...
        public static void PermutePawn(Pawn p1, Pawn p2)
        {
            try
            {
                ...
                //duplication des traits de la source 
                for (int i = 0; i < p1.story.traits.allTraits.Count; i++)
                {
                    Trait trait = p1.story.traits.allTraits[i];
                    Trait nt = new Trait(trait.def, trait.Degree, false);
                    //st1.traits.GainTrait(nt);
                ...
                //duplication des traits de la source 
                for (int i = 0; i < p2.story.traits.allTraits.Count; i++)
                {
                    Trait trait = p2.story.traits.allTraits[i];
                    Trait nt = new Trait(trait.def, trait.Degree, false);
                ...
                for (int i = 0; i < Find.PlayLog.AllEntries.Count; i++)
                {
                    LogEntry log = Find.PlayLog.AllEntries[i];
                ...
                //Rajout des memoires personalisées
                for (int i = 0; i < p1.needs.mood.thoughts.memories.Memories.Count; i++)
                {
                    Thought_Memory x = p1.needs.mood.thoughts.memories.Memories[i];
                ...
                for (int i = 0; i < p2.needs.mood.thoughts.memories.Memories.Count; i++)
                {
                    Thought_Memory x = p2.needs.mood.thoughts.memories.Memories[i];
                ...
                foreach (var wp in Find.WorldPawns.AllPawnsAlive)
                {
                    ...
                    for (int i = 0; i < wp.needs.mood.thoughts.memories.Memories.Count; i++)
                    {
                        Thought_Memory mem = wp.needs.mood.thoughts.memories.Memories[i];
                        if (mem.otherPawn != null)
                        {
                            if (mem.otherPawn == p1)
                            {
        ...
        public static void ResetCachedIncapableOf(Pawn pawn)
        {
            ...
            //{
            //    IEnumerable<WorkTags> list = WorkTagsFrom(combinedDisabledWorkTags);
        ...
        public static void invertRelations(Pawn p1, Pawn p2)
        {
            try
            {
                ...
                for (int i = 0; i < Find.WorldPawns.AllPawnsAlive.Count; i++)
                {
                    Pawn wp = Find.WorldPawns.AllPawnsAlive[i];
                ...
                {
                    Map map = Find.Maps[i];
                    for (int i1 = 0; i1 < map.mapPawns.AllPawns.Count; i1++)
                    {
                        Pawn colon = map.mapPawns.AllPawns[i1];
        ...
        {
            for (int i = 0; i < Find.World.worldObjects.Caravans.Count; i++)
            {
                RimWorld.Planet.Caravan c = Find.World.worldObjects.Caravans[i];
                IList<Thing> list = c.pawns;
                for (int i1 = 0; i1 < list.Count; i1++)
                {
                    Pawn p = (Pawn)list[i1];
                    if (!p.Dead
        ...
        public static Pawn spawnCorpseCopy(Pawn pawn, bool kill = true)
        {
            ...
            {
                List<ThingWithComps> list = pawn.equipment.AllEquipmentListForReading.FastToList();
                for (int i = 0; i < list.Count; i++)
                {
                    ThingWithComps e = list[i];
                    try
                    {
        ...
        public static GameComponent TryGetGameCompByTypeName(string typeName)
        {
            ...
            {
                var comp = Current.Game.components[i];
                if (comp.GetType().Name == typeName)
                {
                    return comp;
                }
        ...
        {
            HashSet<Trait> toDel = null;
            for (int i = 0; i < mind.story.traits.allTraits.Count; i++)
            {
                Trait t = mind.story.traits.allTraits[i];
                if (BlacklistMindTraits.Contains(t.def.defName))
                {
                    if (toDel == null)
                        toDel = new HashSet<Trait>();
                    toDel.Add(t);
                }
            ...
            if (toDel != null)
            {
                mind.story.traits.allTraits.RemoveAll(x => toDel.Contains(x));
            }
        }
        ...
        public static void removeSimpleMindedTrait(Pawn cpawn)
        {
            ...
            {
                Trait toDel = null;
                for (int i = 0; i < cpawn.story.traits.allTraits.Count; i++)
                {
                    Trait t = cpawn.story.traits.allTraits[i];
        ...
        public static void changeHARCrownType(Pawn pawn, string type)
        {
            ...
            if (alienComp != null)
            {
                _ = (string)Traverse.Create(alienComp).Field("crownType").GetValue();
